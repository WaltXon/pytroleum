...
def permits_by_approved_date(
    states, min_approved_date, max_approved_date, url=permits_base_url
):
    print(
        "begin permits request from {} to {}".format(
            min_approved_date, max_approved_date
        )
    )
    url += r"&state_province={}&min_approved_date={}&max_approved_date={}".format(
        states, min_approved_date, max_approved_date
    )
    print("request url: {}".format(url))
    data = api_request(url)
    print("request returned {} values".format(len(data)))
