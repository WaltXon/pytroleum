...
from decline import (
    rate_at_t,
    cum_at_t,
    effective_decline_annual,
    nominal_decline_annual,
)
...
def volumes():
    ...
    df["standard_time"] = df["days_since_start"] / (
        config["standard_days_in_year"] / 12.0
    )
    ...
    volumes_template = {
        d[0]: {"days_in_month": d[1], "days_since_start": d[2], "standard_time": d[3],}
        for d in zipped
    }

    volumes = OrderedDict(
        sorted(
            volumes_template.items(),
            key=lambda t: datetime.datetime.strptime(t[0], "%Y-%m-%d"),
        )
    )
    ...
    for product in products:
        ...
        for date, row in volumes.iteritems():
            ...
            row["period_end_rate_{}".format(product)] = rate_at_t(
            ...
            row["gross_volume_{}".format(product)] = (
                row["cum_prod_{}".format(product)] - cumulative[-1]
            )
            ...
            row[
                "effective_decline_annual_{}".format(product)
            ] = effective_decline_annual(
